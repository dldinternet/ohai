...
describe Ohai::Runner, "run_plugin" do
  describe "when running a plugin with no dependencies, Ohai::Runner" do
    before(:each) do
      ...
      @name = :Thing
      klass = Ohai.plugin(@name) { provides("thing"); collect_data { thing(Mash.new) } }
  ...
  describe "when running a plugin with one dependency" do
    ...
    describe "when the dependency does not exist" do
      before(:each) do
        @name = :Thing
        klass = Ohai.plugin(@name) { provides("thing"); depends("other_thing"); collect_data { thing(other_thing) } }
        @plugin = klass.new(@ohai, "/tmp/plugins/thing.rb")
      end
    ...
    describe "when the dependency has a single provider" do
      before(:each) do
        @name1 = :Thing
        @name2 = :Other
        klass1 = Ohai.plugin(@name1) { provides("thing"); collect_data { thing("thang") } }
        klass2 = Ohai.plugin(@name2) { provides("other"); depends("thing"); collect_data { other(thing) } }
    ...
    describe "when the dependency has multiple providers" do
      before(:each) do
        ...
        @plugins = []
        [@name1, @name2].each do |name|
          @plugins << Ohai.plugin(name) { provides("this"); collect_data { thing(Mash.new) } }.new(@ohai, "/tmp/plugins/#{name.to_s.downcase}.rb")
        end
        @plugins << Ohai.plugin(@name3) { provides("other"); depends("thing"); collect_data { other(thing) } }.new(@ohai, "/tmp/plugins/#{@name3.to_s.downcase}.rb")
        @plugin1, @plugin2, @plugin3 = @plugins
      end

      after(:each) do
        [@name1, @name2, @name3].each do |name|
          Ohai::NamedPlugin.send(:remove_const, name)
        end
      end
  ...
  describe "when running a plugin with many dependencies" do
    before(:each) do
      ...
      @name2 = :Two
      @name3 = :Three
      klass1 = Ohai.plugin(@name1) { provides("one"); collect_data { one(1) } }
      klass2 = Ohai.plugin(@name2) { provides("two"); collect_data { two(2) } }
      klass3 = Ohai.plugin(@name3) { provides("three"); depends("one", "two"); collect_data { three(3) } }
  ...
  describe "when a cycle is detected" do
    before(:each) do
      ...
      @name1 = :Thing
      @name2 = :Other
      klass1 = Ohai.plugin(@name1) { provides("thing"); depends("other"); collect_data { thing(other) } }
      klass2 = Ohai.plugin(@name2) { provides("other"); depends("thing"); collect_data { other(thing) } }
  ...
  describe "when A depends on B and C, and B depends on C" do
    before(:each) do
      ...
      @nameB = :B
      @nameC = :C
      klassA = Ohai.plugin(@nameA) { provides("A"); depends("B", "C"); collect_data { } }
      klassB = Ohai.plugin(@nameB) { provides("B"); depends("C"); collect_data { } }
      klassC = Ohai.plugin(@nameC) { provides("C"); collect_data { } }
    ...
    after(:each) do
      [@nameA, @nameB, @nameC].each do |name|
        Ohai::NamedPlugin.send(:remove_const, name)
      end
    end
...
describe Ohai::Runner, "#cycle_sources" do
  before(:each) do
    ...
    @name2 = :Two
    @name3 = :Three
    klass1 = Ohai.plugin(@name1) { provides("one"); depends("two"); collect_data { one(two) } }
    klass2 = Ohai.plugin(@name2) { provides("two"); depends("one"); collect_data { two(one) } }
    klass3 = Ohai.plugin(@name3) { provides("three"); depends("two"); collect_data { three(two) } }
  ...
  it "should return the sources for the plugins in the cycle, when given an exact cycle" do
    ...
    sources.should eql([@plugin1.file, @plugin2.file])
  end
